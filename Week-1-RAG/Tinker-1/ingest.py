...
# My own code to print out a couple chunks to see what they look like
if __name__ == "__main__":
    ...
    print(f"\nTotal chunks generated across all rule books: {len(all_chunks)}")
    
    # Print out the first 3 chunks to see what they look like
    print("\n--- SAMPLE CHUNKS ---")
    for i, chunk in enumerate(all_chunks[:6]):
        print(f"\nChunk {i+1} (Game: {chunk['game']} | ID: {chunk['chunk_id']})")
        print("-" * 40)
        print(chunk["text"])
        print("-" * 40)
