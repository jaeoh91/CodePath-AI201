...
```
I generated some code to print out the first n chunks to look at what the chunks would look like with our 300-character sliding window strategy would look like. The description in the design docs gave me the impression that this was a pretty robust approach, but I was surprised that a lot of the chunks "cut out" in the middle of sentences and sometimes even in the middle of words. 

Imo, this does reveal an inherent limitation of fixed-character chunking strategies. However, the app is able to work with this strategy as is, because later in the retrieval step, all chunks that are similar enough to the prompt will get retrieved, and in the generation step, it will get combined for the LLM to ingest.
```
